...
while True:
	# buat link
	link = f"https://www.idx.co.id/umbraco/Surface/ExchangeMember/GetBroker?start={start}&length={length}"
	...
	if result["data"] == []:
		...
		for data in result["data"]:
			# simpan semuanya
			broker_code.append(data["Code"])
			broker_name.append(data["Name"])
...
# bentuk dataframe
emiten = pd.DataFrame({ 'code': broker_code,
						'name': broker_name,
					})
...
# save as csv
emiten.to_csv('data/allbroker.csv', index=False)
